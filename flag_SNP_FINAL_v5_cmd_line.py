...
def main():
    ...
    parser = OptionParser()
    parser.add_option("-f", "--file", action="store",  dest="filename", help="write report to FILE")
    parser.add_option("-s", "--size",action="store",  dest="size_stretch", default=5 )
    parser.add_option("-w", "--window", action="store",  dest="window_size", default=10)
    parser.add_option("-r", "--reference",action="store",  dest="reference_fasta", default="Reference_SARS-CoV-2.fasta")
    parser.add_option("-p", "--protected",action="store",  dest="protected_bed", default="")
    parser.add_option("-l", "--look",action="store",  dest="pos_look_bed", default="")
    parser.add_option("-o", "--output",action="store",  dest="output_file", default="output")


    (options, args) = parser.parse_args()
    option_dict = vars(options)

    number_N=int(option_dict['size_stretch'])
    nuc_interval=int(option_dict['window_size'])

    motif="N" * number_N
    print("motif to find "+motif)

    #fasta of the alignement
    fasta_file=option_dict['filename']
    records = list(SeqIO.parse(fasta_file, "fasta"))
    
    #Reference fasta file
    reference_file=option_dict['reference_fasta']
    ...
    bed_file=option_dict['pos_look_bed']
    if(bed_file!=""):
    ...
    protected_file=option_dict['protected_bed']
    if(protected_file!=""):
    ...
    output_file=option_dict['output_file']
    ...
    if(os.path.isfile(output_file + ".txt")):
        print("\n")
        print("*************File "+ output_file + ".txt" " already exist.*************")
        print("\n")
        sys.exit(0)
        #os.remove(output_file+".txt")
    if(os.path.isfile(output_file + ".fasta")):
        print("\n")
        print("*************File "+ output_file + ".fasta" + " already exist.*************")
        print("\n")
        sys.exit(0)
        #os.remove(output_file+".fasta")
    if(os.path.isfile(output_file + ".stats")):
        print("\n")
        print("*************File "+ output_file + ".stats"+ " already exist. *************")
        print("\n")
        sys.exit(0)
        #os.remove(output_file + ".stats")
    if(os.path.isfile(output_file + "_nb_flag_per_seq.txt")):
        print("\n")
        print("*************File "+ output_file + "_nb_flag_per_seq.txt"+ " already exist. *************")
        print("\n")
        sys.exit(0)
    ...
    print("TESTTTTT COMAND LIN")
    print(number_N, nuc_interval, fasta_file, reference_file, bed_file, protected_file,output_file)
    
    #loop through all the sequences in the fasta file 
    for sequence in records:
        
        #check if the sequence has the right length
        if(len(sequence.seq)!= len(reference_seq) ):  #29903):
